...
namespace UploadExcelToSQLServer
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string excelFile = @"C: \Users\bhoward\Documents\finished_goods_20201229.xlsx";
            ...
            using (var db = new DBRasSsrsProd())
            {
                ...
                foreach (DataTable table in result.Tables)
                {
                    ...
                    {
                        Lowes_Daily_List lowesInvRecord = new Lowes_Daily_List()
                        {
                            PartNum = Convert.ToString(dr[0]).Truncate(50), //Part_Number
                            skunum = Convert.ToString(dr[0]).Truncate(50),
                            salesUnit = 5,
                            Description = Convert.ToString(dr[246]).Truncate(100),
                            status = "Active",
                            Company = "RAS",
                            Qty = 50
                        };
                        db.Lowes_Daily_List.Add(lowesInvRecord);
                    }
                }
                db.SaveChanges();
            }
